// routes/post.js
import express from "express";
import multer from "multer";
import Post from "../models/Post.js";
import PostView from "../models/PostView.js";
import auth from "../middleware/authMiddleware.js";
import authOptional from "../middleware/authOptional.js";

const router = express.Router();

/* -------------------- uploads (multer) -------------------- */
const storage = multer.diskStorage({
  destination: (_req, _file, cb) => cb(null, "uploads/"),
  filename: (_req, file, cb) => cb(null, `${Date.now()}-${file.originalname}`),
});
const upload = multer({ storage });

/* ======================= FEED ======================= */
/** Public newest feed */
router.get("/", async (_req, res) => {
  try {
    const posts = await Post.find({})
      .populate("userId", "username avatarUrl")
      .sort({ createdAt: -1 });
    res.json(posts);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/** Optional: single post detail (public) */
router.get("/:id", async (req, res) => {
  try {
    const p = await Post.findById(req.params.id)
      .populate("userId", "username avatarUrl")
      .populate("comments.userId", "username avatarUrl");
    if (!p) return res.status(404).json({ msg: "Post not found" });
    res.json(p);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/* ======================= CREATE ======================= */
/**
 * Create a post (auth)
 * - multipart/form-data with field "media" (image/* or video/*) + "caption"
 * - OR application/json { caption } for text-only posts
 */
router.post("/", auth, upload.single("media"), async (req, res) => {
  try {
    const file = req.file || null;
    const mediaUrl = file ? `uploads/${file.filename}` : null;
    const mediaType = file
      ? file.mimetype.startsWith("video")
        ? "video"
        : "image"
      : undefined;

    const post = await Post.create({
      userId: req.user.id,
      caption: req.body.caption || "",
      mediaUrl,
      mediaType,
    });

    const populated = await post.populate("userId", "username avatarUrl");
    res.status(201).json(populated);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/* ======================= LIKES ======================= */
router.post("/:id/like", auth, async (req, res) => {
  try {
    const p = await Post.findById(req.params.id);
    if (!p) return res.status(404).json({ msg: "Post not found" });

    const idx = p.likes.findIndex((u) => u.toString() === req.user.id);
    let liked;
    if (idx !== -1) {
      p.likes.splice(idx, 1);
      liked = false;
    } else {
      p.likes.push(req.user.id);
      liked = true;
    }

    await p.save();
    res.json({ liked, likes: p.likes.length });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/* ======================= COMMENTS ======================= */
/** Add comment (auth) */
router.post("/:id/comment", auth, async (req, res) => {
  try {
    const p = await Post.findById(req.params.id);
    if (!p) return res.status(404).json({ msg: "Post not found" });

    const text = (req.body.text || "").toString().slice(0, 2200).trim();
    if (!text) return res.status(400).json({ msg: "Empty comment" });

    p.comments.push({ userId: req.user.id, text, createdAt: new Date() });
    p.commentsCount = (p.commentsCount || 0) + 1;

    await p.save();
    await p.populate("comments.userId", "username avatarUrl");

    res.json({
      comments: p.comments,
      commentsCount: p.commentsCount,
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/** Get comments (public) */
router.get("/:id/comments", async (req, res) => {
  try {
    const p = await Post.findById(req.params.id)
      .select("comments commentsCount")
      .populate("comments.userId", "username avatarUrl");
    if (!p) return res.status(404).json({ msg: "Post not found" });

    res.json({
      comments: p.comments,
      commentsCount: p.commentsCount ?? p.comments.length,
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/* ======================= REACTIONS ======================= */
const EMOJIS = ["❤️", "🔥", "😂", "😮", "😍", "💯"];

router.post("/:id/react", auth, async (req, res) => {
  try {
    const { type } = req.body;
    if (!EMOJIS.includes(type))
      return res.status(400).json({ msg: "Invalid reaction" });

    const p = await Post.findById(req.params.id);
    if (!p) return res.status(404).json({ msg: "Post not found" });

    const mine = p.reactions.find((r) => r.userId.toString() === req.user.id);

    if (!mine) {
      p.reactions.push({ userId: req.user.id, type, createdAt: new Date() });
    } else if (mine.type === type) {
      // toggle off
      p.reactions = p.reactions.filter(
        (r) => r.userId.toString() !== req.user.id
      );
    } else {
      mine.type = type;
    }

    // build summary as a plain object (safer with Mongoose schema)
    const summary = EMOJIS.reduce((acc, e) => ({ ...acc, [e]: 0 }), {});
    for (const r of p.reactions) summary[r.type] = (summary[r.type] || 0) + 1;
    p.reactionsSummary = summary;

    await p.save();
    res.json({
      reactions: p.reactions,
      reactionsSummary: summary,
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/* ======================= VIEWS ======================= */
/** one view per day per user (or per IP if not logged in) */
router.post("/:id/view", authOptional, async (req, res) => {
  try {
    const postId = req.params.id;
    const p = await Post.findById(postId);
    if (!p) return res.status(404).json({ msg: "Post not found" });

    const ip =
      (req.headers["x-forwarded-for"]?.toString().split(",")[0] ?? "") ||
      req.socket?.remoteAddress ||
      req.ip ||
      "0.0.0.0";

    const key = req.user?.id ? `u:${req.user.id}` : `ip:${ip}`;
    const day = new Date().toISOString().slice(0, 10).replace(/-/g, ""); // YYYYMMDD

    // Unique per (postId, key, day) — PostView should have a unique index for this trio
    try {
      await PostView.create({ postId, key, day });
      await Post.updateOne({ _id: postId }, { $inc: { viewsCount: 1 } });
    } catch {
      // duplicate key -> already counted today; ignore
    }

    const updated = await Post.findById(postId).select(
      "viewsCount commentsCount reactionsSummary likes"
    );

    res.json({
      viewsCount: updated?.viewsCount || 0,
      commentsCount: updated?.commentsCount ?? 0,
      reactionsSummary: updated?.reactionsSummary || {},
      likes: Array.isArray(updated?.likes) ? updated.likes.length : 0,
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

export default router;